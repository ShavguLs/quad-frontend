      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-12 mb-12 md:mb-20 border-l-4 md:border-l-8 border-[#FFFF2E] pl-4 md:pl-8">
          <div>
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-gray-600 mt-6 italic">Secure Collection Vault V4.2</p>
          </div>
        ...
        <div className="min-h-[500px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-40 gap-6 opacity-30">
              ...
              <span className="text-xs font-black uppercase tracking-[0.5em]">Syncing library...</span>
            </div>
          ) : error ? (
            <div className="p-20 border-4 border-red-600/20 bg-red-600/5 text-center space-y-6">
              <AlertCircle className="w-16 h-16 text-red-600 mx-auto" />
              <h3 className="text-3xl font-black uppercase">Sync Failed</h3>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">{error}</p>
              <button onClick={fetchLibrary} className="bg-red-600 text-white px-8 py-4 text-[10px] font-black uppercase hover:bg-white hover:text-red-600 transition-all">Retry Sync</button>
            </div>
            ...
            <div className="p-24 border-4 border-dashed border-white/10 text-center space-y-8 opacity-40">
              ...
              <div>
                <h3 className="text-3xl font-black uppercase tracking-tighter">Library Is Empty</h3>
                <p className="text-[10px] font-black uppercase tracking-[0.4em] mt-4">No purchased books found in this node</p>
              </div>
              <button onClick={onBack} className="bg-white text-black px-12 py-5 text-xs font-black uppercase hover:bg-[#FFFF2E] transition-all">[ Browse Archive ]</button>
            </div>
          ) : (
            <>
              {viewMode === 'grid' ? (
                      ...
                      >
                        <div className="aspect-[3/4] relative overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-700">
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 backdrop-blur-sm">
                            ...
                            >
                              ...
                              {userReviews[book.id] ? 'Edit Review' : 'Rate'}
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="p-6 space-y-3 bg-zinc-950">
                          <div className="flex justify-between items-start">
                             <div className="space-y-1">
                               ...
                               <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                                  <Clock className="w-3 h-3 text-[#FFFF2E]" /> {(book.created_at || book.createdAt) ? new Date(book.created_at || book.createdAt).toLocaleDateString() : 'Date unavailable'}
                                </span>
                             </div>
                          </div>
                          ...
                          <div className="pt-4 border-t border-white/5 flex justify-between items-center">
                             <span className="text-[8px] font-black text-gray-700 uppercase tracking-widest">Book ID: {book.id.toString().slice(-8)}</span>
                             <div className="flex items-center gap-2">
                                {userReviews[book.id] ? (
                                  <span className="text-[8px] font-black uppercase text-[#FFFF2E]">Reviewed</span>
                                ) : (
                                  <span className="text-[8px] font-black uppercase text-gray-600">Not Reviewed</span>
                                )}
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-1 text-center md:text-left">
                          ...
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-[9px] font-black text-gray-600 uppercase">
                            <span className="flex items-center gap-2"><FileText className="w-3 h-3" /> Book ID: {book.id}</span>
                            <span className="flex items-center gap-2"><Clock className="w-3 h-3" /> Purchased: {(book.created_at || book.createdAt) ? new Date(book.created_at || book.createdAt).toLocaleDateString() : 'Date unavailable'}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          >
                            <Star className="w-4 h-4" /> {userReviews[book.id] ? 'Edit Review' : 'Rate'}
                          </button>
                        </div>
                      </motion.div>
                    ))}
        ...
        <div className="mt-32 pt-12 border-t-2 border-white/5 opacity-20 flex justify-between items-center text-[8px] font-black uppercase tracking-[0.4em] text-gray-600">
           <span>SYNDICATE_ASSET_VAULT_ENCRYPTION_ACTIVE</span>
           <span>V.9.4.0 // END_TO_END_SYNC</span>
        </div>
      </div>
    </div>
  );
