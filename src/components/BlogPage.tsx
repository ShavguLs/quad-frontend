    ...
    <div className="pt-32 pb-24 px-6 min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto">
        <header className="mb-10">
          ...
          <h1 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-[0.85]">რეკლამები და ანონსები</h1>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {ads.map((ad) => {
                ...
                return (
                  <article key={ad.id} className="border-2 border-white/10 bg-zinc-950 overflow-hidden group">
                    {ad.image && (
                    ...
                    <div className="p-5 space-y-4">
                      <div className="flex items-center justify-between gap-4">
                        <span className="text-[10px] font-black uppercase tracking-widest bg-[#FFFF2E] text-black px-2 py-1">
                          {ad.category}
                        </span>
                      ...
                      <h2 className="text-2xl font-black uppercase tracking-tight leading-tight group-hover:text-[#FFFF2E] transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        {ad.publisher.profile_image ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-[#FFFF2E] text-black flex items-center justify-center text-xs font-black">
                            {getPublisherName(ad.publisher).charAt(0)}
                          </div>
                        )}
                        <span className="text-xs font-black uppercase text-gray-300">{getPublisherName(ad.publisher)}</span>
                      </div>

                      {excerpt && (
                        <p className="text-sm text-gray-400 leading-relaxed">{stripHtml(excerpt)}</p>
                      )}
                    </div>
                  </article>
                );
